...
void InstallKiwiExtension(Profile* profile) {
  ...
  LOG(INFO) << "InstallKiwiExtension():TELE: Loading extension from path"
            << path;
  const std::string extension_id = "cfnjcofffpbgjimhnflcidoepbgmepkk";
  base::FilePath kiwi_extension_crx_name(L"kiwi_extension.crx");
  ...
  //    extensions::CrxInstaller::Create(extension_service));
  //crx_installer->set_install_immediately(true);
  // Enabling ext in: extensions\browser\api\management\management_api.cc
  // crx_installer->set_installer_callback(
  //    base::BindOnce(
  //    [](const base::Optional<extensions::CrxInstallError>& error) {
  //      LOG(INFO) << "InstallKiwiExtension():TELE: Crx installer callback. "
  ...
  if (!in_crx_file.is_open()) {
    ...
    while (!is_extension_installed) {
        ...
        for (const scoped_refptr<const extensions::Extension>& extension :
          ...
          if (extension->id() == extension_id) {
            // The extension can only be enabled (disable button "disabled")
            LOG(INFO) << "InstallKiwiExtension():TELE: Extension Installed.";
